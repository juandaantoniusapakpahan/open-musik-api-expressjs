...
exports.dislikeSong = BigPromise(async (req, res, next) => {
  try {
    const { songId } = req.params;
    ...
    if (resultCheckLike.rows.length < 1) {
      ...
      const createdAt = new Date().toISOString();
      queryLike = "INSERT INTO song_ld VALUES($1, $2, $3, $4, $5)";
      const array = [id, songId, userId, "dislike", createdAt];
      arryLike.push(...array);
    }
    ...
    if (resultCheckLike.rows.length > 0) {
      queryLike = "DELETE FROM song_ld WHERE song_id = $1 AND user_id = $2";
      arryLike.push(songId, userId);
      if (resultCheckLike.rows[0].status === "like") {
        queryLike =
          "UPDATE song_ld SET status = $3 WHERE song_id = $1 AND user_id = $2";
        arryLike.push("dislike");
      }
    }
    const query = {
      text: queryLike,
